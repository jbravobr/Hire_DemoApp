...
namespace ArcTouch.TestApp
{
    ...
    public class Results : BaseEntity
    {
        ...
        [Ignore]
        [JsonIgnore]
        public Color RoundedLabelColor
        {
            get
            {
                if (!string.IsNullOrEmpty((ReleaseDate)))
                    return Convert.ToDateTime(ReleaseDate).Year == 2017 ? Styles.FeaturedThisYear : Styles.ComplementColor;

                return Styles.ComplementColor;
            }
        }
